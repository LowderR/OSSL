...
class DataGenerator:
    ...
    def build_dataset(self, data_type):
        ...
        # 将数据按照长度排序    eval() 函数用来执行一个字符串表达式，并返回表达式的值。
    ...
    def inject_outliers(self, ratio=0.25, level=5, point_prob=0.3, vary=False, data_type=None):
        ...
        # np.random.choice中repalce可以选择生成的随机数是否重复,相当于生成size个np.randint(0, traj_num)不重复的数
    ...
    def load_outliers(self, data_type=None, resample=False, novel_type=1, eta=0.5):
        ...
        labeled_classes = [0, 1, 2]
        unlabeled_classes = [3, -1]
        ...
        if data_type == 'train':
            trajectories = trajectories + self.val_trajectories
            labels = labels + [0 for i in range(len(self.val_trajectories))]
            filename = '{}_outliers_{}.pkl'.format(self.dataset, 'val')
            with open('./data/{}/NCD3/detour_'.format(self.dataset) + filename, 'rb') as fp:
                detour_idx = pickle.load(fp)
            for idx, o in detour_idx.items():
                trajectories[idx] = o
        ...
        if data_type == 'train':
            ...
            if resample == True:
                ...
                with open('./data/{}/NCD3/aug_trainval_trajectories_{}.pkl'.format(self.dataset, eta), 'wb') as fp:
                ...
                with open('./data/{}/NCD3/aug_trainval_trajectories_{}.pkl'.format(self.dataset, eta), 'rb') as fp:
