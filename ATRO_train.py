...
import numpy as np
from ATRO_dataloader import DataGenerator
...
from eval import Valid_train, Valid_val, Valid_binary_val

# args
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
# 1:val=GPS  2:val=detour  3:val=switching
novel_type = 1
novel_map = {1: 'navigation', 2: "detour", 3: "switching"}
dataset = 'porto'
# dataset = 'chengdu'
...
embedding1 = []
embedding2 = []
with open('./embedding/{}/embedding_osr{}_SD.csv'.format(dataset, novel_type), 'r') as f:
    for line in f:
        if (a1 == 0):
            a1 += 1
        else:
...
with open('./embedding/{}/embedding_osr{}_region.csv'.format(dataset, novel_type), 'r') as f:
    for line in f:
        if (a2 == 0):
            a2 += 1
        else:
...
def Train(model, data, optimizer, simnet, optimizer_simnet):
    ...
    model.train()
    total_loss = 0
    for batch_encode_input, _, _, batch_seq_length, labels, _ in data.iterate_train_data():
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        Valid_binary_val(model, data, valid_batch_size, device, num_labels)
